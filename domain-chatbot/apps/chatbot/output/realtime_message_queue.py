...
def realtime_callback(role_name: str, you_name: str, content: str, end_bool: bool):
    ...
    # if re.match(r"^(.+[。．！？\n]|.{30,}[、,])", realtime_callback.message_buffer) or end_bool:
    if (
        len(realtime_callback.message_buffer) > 30 and re.search(r"[。．！？\n]", content)
    ) or end_bool:
        ...
        emote = generation_emote.generation_emote(query=message_text)
...
def realtime_callback2(role_name: str, you_name: str, content: str, expand:str):
    content = format_chat_text(role_name, you_name, content)
    ...
    put_message(
        RealtimeMessage(
            type="user", user_name=you_name, content=message_text, emote=emote, expand=expand
        )
    )
