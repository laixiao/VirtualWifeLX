...
class MemoryStorageDriver:
    ...
    def __init__(
        self, memory_storage_config: dict[str, str], sys_config: SysConfig
    ) -> None:
    ...
    def search_short_memory(
        self, query_text: str, you_name: str, role_name: str
    ) -> list[Dict[str, str]]:
        local_memory = self.short_memory_storage.pageQuery(
            page_num=1, page_size=self.sys_config.local_memory_num, owner=role_name, sender=you_name
        )
        dict_list = []
    ...
    def save(
        self, you_name: str, query_text: str, role_name: str, answer_text: str
    ) -> None:
        # 存储短期记忆
        pk = self.get_current_entity_id()
        local_history = {
            "ai": self.format_role_history(
        ...
        if self.sys_config.enable_longMemory:
            ...
            if self.sys_config.enable_summary:
                ...
                history = memory_summary.summary(
                    llm_model_type=self.sys_config.summary_llm_model_driver_type,
                    input=history,
                )
                # 计算记忆的重要程度
