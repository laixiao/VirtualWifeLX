...
class LocalStorage(BaseStorage):
    ...
    def pageQuery(self, page_num: int, page_size: int, owner: str, sender: str) -> list[str]:
        # 计算分页偏移量
        offset = (page_num - 1) * page_size

        # 分页查询，并提取 text 字段
        results = (
            LocalMemoryModel.objects.filter(owner=owner, sender=sender)
            .order_by("-timestamp")
            .values_list("text", flat=True)[offset : offset + page_size]
        )
        return list(results)

    def save(
        self, pk: int, query_text: str, sender: str, owner: str, importance_score: int
    ) -> None:
