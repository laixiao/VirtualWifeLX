...
class BiliHandler(BaseHandler):
    ...
    async def _on_heartbeat(self, client: BLiveClient, message: HeartbeatMessage):
        ...
        message_body = {"type": "system", "content": "", "cmd": cmd_str}
    ...
    async def _on_like_click(
        self, client: BLiveClient, message: LikeInfoV3ClickMessage
    ):
    ...
    async def _on_interact_word(
        self, client: BLiveClient, message: InteractWordMessage
    ):
        """
        用户进入直播间，用户关注直播间
        """
